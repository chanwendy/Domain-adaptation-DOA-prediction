...
class NiLoader:
    ...
    """
    def __init__(self, root_dir="/HDD_data/HYK/bis/database/ni_dataset", nums=44, time_step=1, tw=180):
    ...
    def clean(self):
        ...
        for i in range(self.nums):
            ...
            # data1.RFTN20_VOL = np.interp(rftn1, source_data_x, source_data_y) / 2  # 浓度标准化处理， vitalDB: 20, NI:40
            #
            # ppf_ce = list(range(5 * len(data1.PPF20_CE.values)))
            # data1.PPF20_CE = np.interp(ppf_ce, source_data_x, data1.PPF20_CE.values)
            # rftn_ce = list(range(5 * len(data1.RFTN20_CE.values)))
            # data1.RFTN20_CE = np.interp(rftn_ce, source_data_x, data1.RFTN20_CE.values)
            #
            # bis = list(range(5 * len(data1.BIS.values)))
            # data1.BIS = np.interp(bis, source_data_x, data1.BIS.values)
...
#ppf0 = xlrd.open_workbook("/HDD_data/HYK/bis/database/ni_dataset/record/ppf/2022-05-30-2.xls")
# rftn0 = xlrd.open_workbook("/HDD_data/HYK/bis/database/ni_dataset/record/rftn/2022-05-30-2.xls")
# ppf0 = ppf0.sheets()[0]
# ppf = ppf0.col_values(6)
# ppf = ppf[4:-1]
# ppf_times = ppf0.col_values(8)
# ppf_times = ppf_times[4:-1]
# rftn0 = rftn0.sheets()[0]
# rftn = rftn0.col_values(6)
# rftn = rftn[4:-1]
# rftn_times = rftn0.col_values(8)
# rftn_times = rftn_times[4:-1]
# ni0 = xlrd.open_workbook("/HDD_data/HYK/bis/database/ni_dataset/label/2022-5-30-2NI.xls")
# ni0 = ni0.sheets()[0]
# ni = ni0.col_values(2)
# ni = ni[4:-1]
# ni_times = ni0.col_values(0)
# ni_times = ni_times[4:-1]

# a3,b3,c3 = crop(ppf,rftn,ni,ppf_times, rftn_times, ni_times)
